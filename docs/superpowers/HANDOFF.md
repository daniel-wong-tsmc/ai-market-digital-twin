# HANDOFF — GPU Category Agent (resume point: **2026-08-10 — DAILY CYCLE 2026-08-v6 RUN + COMMITTED + PUSHED `80e54f0` (scheduled headless, live gather; `main == origin/main`). Scorecard `store/chips.merchant-gpu/2026-08-v6.json` — **DMI 3.780 / SMI 0.400** (prior v5: 3.46 / −0.20); Strong / improving, binding constraint "Stacked memory and wafer supply". ★ SMI FLIPS POSITIVE (−0.20 → +0.40, SOFTENING → ACCELERATING) on Samsung + SK hynix HBM4 yields near 80%, four months early. Suite **2441 passed / 5 skipped** after registering v6 in the scoring v1 replay pin (W_CURRENT — the tripwire went red exactly as designed; v6 replays exactly); all four pins green (60). Run health: 10 docs kept (4 primary / 6 secondary), 1 gather round, 0 already-known; L2 dedup new 2 / update 14 / duplicate 14; 30 findings gated, 2 dropped by the gate; voice-lint + sufficiency clean, ZERO bypasses. ★ **F113's FIRST LIVE TEST — PASSED AS DESIGNED:** chart-research emitted 2 prompts; bullet-2 returned an honest NO-SERIES-FOUND; bullet-1's candidate was REJECTED WHOLE by the verifier because the researcher handed back the source's hedged ranges ('below 60%', 'close to 80%') where numbers are required. Quarantine held — nothing unverified reached `store/`, `research-series/` still does not exist. Follow-up worth filing: the cited article publishes only ranges, so NO-SERIES-FOUND was the honest answer for bullet-1 too; the researcher brief could say outright that a hedged range is not a plottable number. **F114 day-2 checks MET:** 3 narrator-authored bullets, each with a digit, on the page. ⚠ **THESIS STAGE FAILED** (`thesis: failed`) after its full 3-dispatch allowance — attempts 1 and 2 were schema misses (a missing required `sensitivity`), attempt 3 was schema-valid (63 judgments + 2 proposals) but voice-lint rejected ONE proposed thesis for a raw finding id in exec-facing prose plus a 2-sentence statement. The gate never wrote, so the thesis book is byte-unchanged and THE CALLS renders the PRIOR cycle's book; the scorecard is unaffected. ⚠ **`impl:7` FLAGGED AGAIN** by the citation audit (3 uncited numbers: 210000, 4.55, 9.66 — the same China-revenue pair as the 2026-08-05 v2 cycle); all narrator claims clean, so per run-cycle 3(e4) it was logged, not re-dispatched. Still needs a human look. **FIVE AFK-defaults recorded this run** (see the dated bullet + the cycle log's `deviations`): the unchanged tool-less-brain deviation, the prompt-splitting workaround, last30days skipped on cap, a NEW gatherer tool-wall deviation (F88's no-Bash wall was instructed, not structurally enforced — no subagent type exposes that exact tool set), and a NEW F67 session-output deviation (the 1021-line / 109KB report was summarised in the session, not pasted verbatim). NEXT: the F115 build lane is still claimed and unstarted; then the multi-category rollout conversation. Carried: v5's 4 AFK-defaults, impl:7, Step 3(b) ruling + earnings-day timing, F91b, F92, F90, F111 grain, F112 minors, F79-G4 soak.** Prior state: 2026-08-10 — F115 ISSUE TRACKER DESIGNED + PLANNED, docs-only, pushed `94a5885` (`main == origin/main`). Interactive brainstorm, every fork a user pick (ZERO AFK-defaults): a persistent "Known issues" tracker at the bottom of the category page — agent-minted via deterministic triggers (binding constraint; weak+worsening dimension), narrator-assessed each cycle with reasoning (GATED: narrator prompt + pin re-record in lockstep, exclusive prompt lane), resolve after 5 consecutive good cycles, register + append-only history (`store/<cat>/issues/`), dashboard schema 1.1→1.2 + new React `Issues` section, run-cycle sub-steps (d4)/(e3b) + F83 re-record in-lane. Spec `995ff17` (`docs/superpowers/specs/2026-08-10-f115-issue-tracker-design.md`), 10-task plan `94a5885` (`docs/superpowers/plans/2026-08-10-f115-issue-tracker.md`). NEXT: execute F115 subagent-driven in `.worktrees/f115-issue-tracker` (user chose handing to a fresh instance; kickoff prompt emitted). ⚠ NO DAILY CYCLE RAN 2026-08-09 or 08-10 (no v6 scorecard / story / work dir; origin unmoved) — the scheduled task silently missed two days (the F90 "silently missed runs" pattern, recurring); F113's FIRST LIVE TEST and F114's day-2 checks are therefore STILL PENDING, and the offered manual chart-research run remains OFFERED-NOT-RUN. Suite verified 2026-08-08 this session pre-docs: 2440 passed / 5 skipped; only docs committed since. Carried: v5 cycle's 4 AFK-defaults, impl:7, Step 3(b) ruling + earnings-day timing, F91b, F92, F90, F111 grain, F112 minors, multi-category rollout, F79-G4 soak.** Prior state: 2026-08-08 (afternoon) — ★ F114 LIVE CRITERION MET on its first live day: the 08:57 v5 cycle's story artifact is schemaVersion 2 with 3 narrator-authored bullets, live on the page (concrete, self-contained, numbers in each). **F113 NOT YET EXERCISED — merged AFTER the 08:57 run** (v5 journal has NO chart-research entry), so today's page has zero charts by TIMING, not failure: with no researcher, all three bullets fell back to estimate-grade series which the quality rule refuses to draw (3× estimate-only causes, correct). **F113's first live test = the next scheduled run (daily task fires 08:57; verified Ready, last result 0).** An OFFERED-NOT-RUN option stands: manually run the chart-research step against today's 3 bullets (memory shortfall / H200-B300 street prices / Micron-GlobalWafers) and refresh dashboard.json — needs a user go-ahead. No live run is in flight (verified: task idle, work/ silent since 09:54, tree clean). **Machine housekeeping this session:** F113/F114 worktrees+branches retired (SDD ledgers rescued complete to root first), stale `.worktrees/f110-dashboard` removed after killing its orphaned `vite preview` (PIDs verified); ⚠ TWO ORPHANED ELEVATED python processes from Aug 4 (PIDs 1420 + 7824 — 7824 has 2.7h CPU + live Cloudflare-fronted TCP connections) could NOT be killed (Access denied even unsandboxed — they hold admin tokens): **the user must run `taskkill /PID 1420 /PID 7824 /F` from an ADMIN terminal**; the `headroom mcp serve` python (PID 10932) deliberately left (may serve still-open Aug 4-5 claude terminals). `main == origin/main`; suite 2440 passed / 5 skipped + four pins (59) verified earlier this session. Carried: v5 cycle's 4 AFK-defaults, impl:7, Step 3(b) ruling + earnings-day timing, F91b, F92, F90, F111 grain, F112 minors, multi-category rollout, F79-G4 soak.** Prior state: 2026-08-08 (later) — F114 MERGED `092ef0e` + F113 MERGED `705a5ee` + required data refresh `6b81e9b`; `main == origin/main == 6b81e9b`. Session-verified on merged main: suite **2440 passed / 5 skipped**, all four pins green (59 checks). The narrator now writes the three dashboard bullets (gate check 8, citation audit `bullet:<i>` claims, prompt pin re-recorded exactly once in lockstep `6f353f6`); chartless bullets get a same-day tool-USING chart researcher behind quarantine+verify (all-or-nothing verifier importing the citation audit's number matching; `registry/chart-series.json` has a proven no-writers test); render fixes live (schema 1.1 structured causes, full-width chartless layout, inline source badges, "Found today — single source" label). **LIVE CRITERIA NOT YET MET for either lane** — the 2026-08-08 v5 cycle ran BEFORE the merges, so the live page currently shows mechanical-fallback bullets (3× estimate-only causes) and no `store/chips.merchant-gpu/research-series/` exists yet; the NEXT scheduled cycle is the proof (expect: 3 narrator-authored bullets ≤ 28 words each with a digit; the chart-research step emitting/accepting; possibly the first researched chart). Housekeeping DONE (same session, user-directed): `f114-narrator-bullets` + `f113-chart-researcher` worktrees removed cleanly (no `--force`) + branches deleted; both gitignored SDD ledgers copied COMPLETE to root `.superpowers/sdd/` first (the root F114 copy had been near-empty); the stale `.worktrees/f110-dashboard` leftover was held by an orphaned `vite preview` server running since 08-06 (PIDs 6528/11596, verified by command line before kill) — killed, directory removed. Standing worktrees untouched: `eval-v2`, `f73-canary`, `f79-scoring-v2`. Carried: the v5 cycle's FOUR AFK-defaults (see its bullet, notably the unchanged tool-less-brain deviation + new prompt-splitting workaround), impl:7 flag, Step 3(b) ruling + earnings-day timing, F91b's 8 approvals, F92's 4 decisions, F90's calls, F111 gap-chart grain, F112 small follow-ups, multi-category rollout, F79-G4 soak. NEXT: watch the next scheduled cycle for both live criteria; then the multi-category rollout conversation.** Prior state: 2026-08-08 — DAILY CYCLE 2026-08-v5 RUN + COMMITTED + PUSHED `1ffadc2` (scheduled headless, live gather; DMI 3.46 / SMI -0.20; suite 2363 passed / 5 skipped; narrator done not fellBack; citation audit clean; FOUR AFK-defaults to re-surface — see its dated bullet, notably the unchanged tool-less-brain deviation and a new prompt-splitting workaround).** Prior state: **2026-08-06 (later) — F113 + F114 DESIGNED + PLANNED, docs-only, pushed `2cb5cbc` (`main == origin/main`). User screenshot review of the live F110 page → two follow-ups, all decisions interactive (ZERO AFK-defaults): **F114 narrator-authored bullets** (spec `93759a8`, 6-task plan `2cb5cbc`, GATED exclusive prompt lane — narrator pin re-record, builds FIRST in `.worktrees/f114-narrator-bullets`) and **F113 same-day chart researcher** (spec `93759a8`, 7-task plan `2cb5cbc`, quarantine+verify trust model, render fixes ride along; dispatches ONLY after F114 merges, `.worktrees/f113-chart-researcher`). Suite **2318 passed / 5 skipped** at handoff. NEXT: execute F114 then F113 via subagent-driven-development. See the dated bullet below.** Prior state: 2026-08-06 — F110 DASHBOARD REVAMP BUILT, REVIEWED, MERGED `d62e800` (`--no-ff`) + PUSHED; post-merge data refresh `d29291d`; `main == origin/main == d29291d`. The main merchant-GPU category page is now a verdict-led React 19 + Astryx dashboard fed by a daily `dashboard.json` that the pure-Python cycle exports — Node never enters the scheduled run. Merged-main suite **2318 passed / 5 skipped**; all four pins green (57); only the F83 run-cycle fingerprint moved (Task 7, F109 precedent); forbidden diff EMPTY; zero deletions under `site/chips.merchant-gpu/`. Built subagent-driven: 12 TDD tasks, fresh implementer + per-task spec+quality review each, whole-branch review, ONE final fix wave, 29 commits. **FIVE interactive user decisions during the build, ZERO AFK-defaults** (jsonschema as a real dependency; the mixed dated/monthly gap chart; that choice REAFFIRMED after being shown it compares unlike scales; the dimension legend wording departing from the mock; AMD source link-discovery). **KNOWN USER-ACCEPTED LIMITATION → filed F111:** the gap chart mixes day-grain readings (~0.04–0.23) with month-grain ones (~3.4–3.7), so the line appears to collapse and rebound across the seam and the direction badge can flip on which grain is last; the caption discloses the mix but not that the magnitudes are non-comparable. Small deferred follow-ups → **F112** (notably: AMD link discovery has no staleness check). Records: `docs/superpowers/f110-dashboard-DONE.md`, backlog F110/F111/F112. Worktree + branch `f110-dashboard` retired. **NEXT: the three live criteria are NOT yet met — the next scheduled cycle should write `dashboard.json` with no human step and the live page render it; watch for a bullet gaining a real mini-chart once AMD's next quarterly release lands. Then: the multi-category rollout conversation; F79-G4 soak.** Carried open items unchanged: impl:7 citation flag, the Step 3(b) tool-less-brain ruling + earnings-day timing, F91b's 8 approvals, F92's 4 decisions, F90's calls.** Prior state: 2026-08-05 (evening) — F110 DASHBOARD REVAMP DESIGNED + PLANNED, docs-only, pushed `208ec75`. Spec `ab98537` (`docs/superpowers/specs/2026-08-05-dashboard-revamp-design.md`) + 12-task plan `208ec75` (`docs/superpowers/plans/2026-08-05-f110-dashboard-revamp.md`) + approved visual mock committed at `docs/superpowers/specs/assets/2026-08-05-dashboard-mock.html`. ALL decisions interactive user decisions — ZERO AFK-defaults (incl. the F95 no-script override for the main category page and the future in-lane F83 re-record). NEXT: execute F110 via subagent-driven-development in `.worktrees/f110-dashboard`; suite 2174 passed / 5 skipped at handoff. See the dated bullet below.** Prior state: 2026-08-05 — FIVE PARALLEL LANES MERGED + PUSHED (`306a510`), then the v2 daily cycle landed on top (`febaad4`) and the backlog/handoff sync followed. Lanes: F109 (durable coverage-gap records; **live criterion MET same day** — the v2 cycle wrote `store/chips.merchant-gpu/coverage-2026-08.json` via the new step, no human step), F68 (CLOSED `a723dac` — 5 of 6 items were already shipped; thesis-prose lint now WIRED + BLOCKING, "ASE" allowlisted), F90 (`docs/operator-rebuild.md` DONE), F91b (`docs/publishing-posture.md` DRAFT — 8 approval points OPEN), F92 (retention memo — 4 decisions OPEN, decide-this-month; store grows ~18× faster than the July memo assumed). All five lane designs were interactive user decisions; five worktrees + branches retired cleanly. Suite 2173 passed / 5 skipped at the lane merges; all four pins green (F83 re-recorded by F109 in-lane). ⚠ AWAITING THE USER: (1) the v2 cycle's citation audit flagged `impl:7` — two China-revenue figures with no cited source, logged per the implication rule, needs a human look; (2) the 08-04/08-05 cycles' repeated Step 3(b) tool-less-brain deviation ruling + the earnings-day timing decision; (3) F91b's 8 approvals; (4) F92's 4 decisions; (5) F90's calls (6 silently missed scheduled runs — task settings; 7 machine-local files with no backup — repo-copy?). NEXT: the multi-category rollout conversation; F79-G4 soak (~mid-August).** Prior state: F99 DONE 2026-08-04 — canary re-captured on the first attempt (CATCH: damaged extract 5.375 vs hard bar 5.533, HARD-FAIL naming extract, negatives all ≤ 2), test un-skipped, fixture `fixtures/evals/canary/extract-rules-stripped/`, commit `0acaff9` — see the 2026-08-04 dated bullet. Every decision was an interactive user decision (explicit capture grant given in-session); ZERO AFK-defaults. NEXT: the multi-category rollout conversation; F91(b) posture doc; F79-G4 soak clock runs toward ~mid-August. **F66 live criterion is now MET** — the 2026-08-04 daily cycle (`ce593cc`) wrote `store/chips.merchant-gpu/audit/2026-08-04.json` with `summary.flagged == 0` (12 claims audited); see that dated bullet, which also carries 4 AFK-defaults awaiting the user's ruling (notably a deviation from run-cycle Step 3(b)'s "tool-less brain" wording) and a decision needed on daily-cycle timing vs same-day earnings prints.** Prior state: F107 DIAGNOSED + CLOSED per user decision (a) (single-run outlier; 3 thesis-only replicates, ~23 Opus dispatches, zero bypasses, zero hand-edits) — eval note `docs/superpowers/eval-notes/2026-07-29-f107-thesis-replicates-note.md`, with a LEVEL caveat carried forward (revisit grader-severity-vs-bar ONLY when the thesis prompt next changes). F91 decided (a) — old repo stays public (`8f260d3`). F99 PREPARED + PARKED — package at `.superpowers/handoffs/f99-canary-prep-PACKAGE.md`; user weighing ~35-40 Opus dispatches vs parking, and the cheaper-model question is ANSWERED: cheaper models would invalidate the Opus-calibrated bar, so it is Opus or park. **Every decision in the 2026-07-29 session was an interactive user decision — ZERO AFK-defaults.** F66 citation audit BUILT on `f66-citation-audit` with a complete DONE sentinel, user-authorized to merge this session. **F66 CITATION AUDIT (Phase 1) MERGED `f19d830` (`--no-ff`) + PUSHED — user-authorized; suite 2146 passed / 6 skipped on merged main, all four pins green, forbidden diff empty. Branches + worktrees `f66-citation-audit`, `f79-g4-refresh`, `f106-huggingnews` RETIRED (clean `git worktree remove`, no `--force`). `main == origin/main == f19d830`. NEXT: the user's F99 call (dispatch vs park), then the multi-category rollout conversation; F66's live criterion — the next live cycle runs `(e4)` and writes an audit artifact with `summary.flagged == 0` — is not yet met. See the 2026-07-29 dated bullets immediately below.**) Standing prior state: **DAILY CYCLE 2026-07-v20 RUN + COMMITTED + PUSHED `af46e6c` (2026-07-28, scheduled headless; Strong/improving, DMI 3.447 / SMI -0.967; suite 1988 passed / 6 skipped; narrator done, not fellBack; AFK-defaults to re-surface — see its dated bullet).** Standing prior state: DAILY CYCLE 2026-07-v19 RUN + COMMITTED + PUSHED `797ba90` (2026-07-27, scheduled headless; Strong/improving, DMI 3.420 / SMI -0.780; suite 1987 passed / 6 skipped; narrator done, not fellBack; three AFK-defaults re-surfaced in its dated bullet). Standing prior state: **F96 CONTENT-VINTAGE IDS — BUILT + MERGED `439fa6e` + PUSHED (2026-07-26; main == origin/main == ae337b2 after the F102 merge); F102 PRICE-SYNC MONTH-GRAIN FIX — BUILT + MERGED `ae337b2` (`--no-ff`) + PUSHED 2026-07-26 (see the dated bullet just below)** 2026-07-25/26 — both were designed + planned interactively, docs-only; F96 is now shipped. **F96 (content-vintage ids):** spec `18c0334` + 3-task plan `f4c07ee` — doc-id digest becomes URL+folded-content (reuses `dedup.content_hash`), so a same-month price re-fetch with changed content mints a NEW id that L2 dedup classifies as an update; the append-only store's collision check stays as the tripwire; no migration; user pick: "URL + content vintage". Lane: `.worktrees/f96-content-ids`, branch `f96-content-ids`. **✓ BUILT + MERGED `439fa6e` (`--no-ff`) + PUSHED 2026-07-26; built subagent-driven (3 TDD tasks, fresh implementer + per-task spec+quality review each, whole-branch Opus review = MERGEABLE, 0 critical/important, 2 non-blocking minors); one interactive question-stop resolved (the planned top-level `content_hash` import would cycle with dedup.py — user chose Option A deferred/function-level import, matching the existing publisher.py idiom); merged-main suite 1980 passed / 6 skipped; all four pins (F6 / narrator / scoring-v1-replay / F83) green; forbidden-diff (`fixtures/`/`registry/`/`gpu_agent/evals`/`gpu_agent/narrator`) empty; `gpu_agent/store.py` untouched (tripwire retained); worktree + branch retired; DONE sentinel `.superpowers/handoffs/f96-content-ids-DONE.md`. Live criterion (post-merge, not forced): next monthly-grain price re-gather logs zero id collisions.** **F102 (price-sync month-grain crash):** spec `18694cd` + 1-task plan (this commit) — `_parse_as_of` accepts YYYY-MM-DD and YYYY-MM (month-end anchor via `_month_end_yymmdd`); malformed as-of degrades to the documented warning path (no traceback escapes `sync_series`); scope = `price_local.py` (+ CLI handler only if it re-raises). Lane: `.worktrees/f102-price-grain`, branch `f102-price-grain`. The lanes touch DISJOINT files and may run as parallel instances; neither touches prompts/registry/evals — all four pins must stay green in both. STOP before merge; sentinels `f96-content-ids-DONE.md` / `f102-price-grain-DONE.md`. **✓ THIS SESSION ALSO:** F103 DEPLOYED (`dce8dbd`, pushed — the live page now dates/weight-sorts/aging-dims evidence; suite on merged main **1972 passed / 6 skipped**); **FIRST NARRATED CYCLE VERIFIED** (v17 cycle 2026-07-25: `store/chips.merchant-gpu/story/2026-07-25.json`, fellBack:false, retries:0, 4 scenes, live headline "The chips are ready. The parts to finish them are not." — Phase B's live criterion PASSED). After F96/F102: housekeeping sweep (28 worktrees), F79 G4 + F99, the multi-category rollout conversation. Standing prior state: **F103 FRESHNESS DECAY — MERGED `62676f6` + DEPLOYED `dce8dbd`** 2026-07-24/25; originally READY TO DISPATCH 2026-07-24 — evidence half-life decay (user critique: stale official NVIDIA earnings material keeps surfacing as current) is DESIGNED + PLANNED, docs-only: spec `c6d47c3` (`docs/superpowers/specs/2026-07-24-f103-freshness-decay-design.md`) + 8-task plan `d310f52` (`docs/superpowers/plans/2026-07-24-f103-freshness-decay.md`), backlog F103. **User decisions (interactive, zero AFK):** A+B+C ALL (renderer fixes + narrator prose-dating rule + gather earnings-window cadence) + a per-kind half-life engine, **half-lives user-set: news 3d / filings 5d / structural 45d** (registry/freshness.json, tunable by JSON edit); decay applies EVERYWHERE BUT THE JUDGE (scores comparable; judge-side decay = separate gated follow-up if ratings still skew stale). New `gpu_agent/freshness.py` + validated `registry/freshness.json`; evidence rows weight-sorted/always-dated/publisher-capped/aging-dimmed; Explore findings weight-sorted; narrator gate check 7 (aged claims must be dated in prose — prompt-neutral) early, then Task 7 LAST = inputs `freshnessWeight` annotations + 3 prompt rules + **narrator pin re-record in the same commit** (the ONLY pin that moves; F6/scoring-replay/F83 untouched, forbidden-diff check at every commit). Manifest gains `cadence: earnings-window` + `earningsDates` (model fields added; `primaryDomains` exposure gap closed); gather-category SKILL.md wording (NOT fingerprint-pinned — verified in-plan Task 6 Step 1). Execution: subagent-driven in `.worktrees/f103-freshness`, STOP before merge → `.superpowers/handoffs/f103-freshness-DONE.md`. ⚠ Task 7 re-records the narrator pin: no OTHER narrator-prompt-touching lane may run concurrently. **✓ F101 FULLY CLOSED + DEPLOYED:** Explore-layer deploy `ed5a332` (live); first NARRATED cycle still pending (no `store/<cat>/story/` dir yet — the next scheduled cycle is Phase B's live criterion; if it writes `fellBack:true` or no artifact, investigate per the B sentinel). Handoff-integrity fix `38d7707` (relabeled an off-vocabulary provenance label to "user-approved"). After F103: F96 + F102 fix lanes, F79 G4, multi-category rollout conversation. Standing prior state: **F101 PHASE C — EXPLORE LAYER: BUILT, REVIEWED + MERGED + PUSHED** 2026-07-24 — merge `fcf996a` (`--no-ff`, pushed; **`main == origin/main == fcf996a`**); merged-main suite **1936 passed / 6 skipped**; all four pins (F6 / narrator / scoring replay / F83) GREEN; DONE sentinel `.superpowers/handoffs/f101c-explore-DONE.md`; branch `f101c-explore` + worktree NOT yet retired (housekeeping open). Phase C (the narrative's footnote system: story archive permalinks, question-grouped findings browser, grouped series page, entity dossiers, verdict-timeline history, link-integrity build gate) SHIPPED — renderer/copy layer only; frozen core / scoring / all brain prompts / narrator prompt / evals / eval+narrator fixtures / registry / run-cycle SKILL.md byte-untouched (forbidden-diff empty across the branch). Built subagent-driven (9 build commits `12b2f5f`→`cd8369c`, fresh implementer + per-task spec+quality review each + whole-branch Opus review). **One user-approved design fork (interactive question-stop, NOT AFK):** front-page visible links to the deep pages use DIRECTORY routes (`findings/`, `series/`, `history.html`, `story/`) not `.../index.html`, because the story-copy lint allows one `index` token/page and the gap-chart "indexed" axis label spends it — Option A (leave the lint untouched) chosen over amending the shared gate (`.superpowers/handoffs/f101c-explore-QUESTIONS.md`). **One Important whole-branch finding FIXED (`cd8369c`):** the front-page pre-filtered evidence-panel explore CTA (spec §1 flagship) resolved one directory too high — a dead link INVISIBLE to the link gate (JSON→href via JS); fixed by storing category-root-relative + re-basing `../` per surface in `render_story_day`, plus a new on-disk resolve test closing the gate's blind spot. Deferred minors (in the DONE sentinel): story-permalink prose not entity-linked (permalinks empty today); T4 findings growth-note (YAGNI until >1MB). **NEXT: F96 (finding-id collision, 4th sighting) + F102 (price-sync `_yymmdd_date` month-grain crash) fix lanes (filed `75c775e`), then the multi-category rollout conversation. OPTIONAL: a combined deploy (rebuild `site/` + push, F100 precedent) makes the Explore pages live — the live page still runs the Phase-A/B story page until then.** Phase C was DESIGNED + PLANNED first: spec `c364085` (`docs/superpowers/specs/2026-07-23-f101c-explore-layer-design.md` — §1 governing principle: deep pages are entered FROM INSIDE the story, pre-filtered, breadcrumbed; never a generic data library) + 8-task plan `85b0002` (`docs/superpowers/plans/2026-07-23-f101c-explore-layer.md`). **Its precondition (B merged) is SATISFIED — dispatch freely.** Renderer/copy layer only; all four pins (F6 / narrator / scoring replay / F83) untouched by design. Execution: subagent-driven in `.worktrees/f101c-explore`, STOP before merge → `.superpowers/handoffs/f101c-explore-DONE.md`. **✓ DEPLOY CONFIRMED DONE (this session):** the v16 daily cycle's own step-8 site rebuild (2026-07-24, `0b1e200`, run on post-Phase-A main) already emitted + pushed the story page — a fresh rebuild is byte-identical and the LIVE Cloudflare page verified serving "The GPU shortage got worse this month." / "The story, step by step" / revision 16 (assembled story — no narrated artifact existed pre-B-merge; the FIRST NARRATED entry is expected from the next scheduled cycle, which is also Phase B's live post-merge criterion — check `store/chips.merchant-gpu/story/` + `narrator: done` in the cycle log). After C: F96 + F102 fix lanes (filed `75c775e`), then the multi-category rollout conversation. Standing prior state: **F101 PHASE B — DAILY NARRATOR: BUILT, REVIEWED + MERGED + PUSHED** 2026-07-24 — merge `3e1049e` (`--no-ff`, pushed; **`main == origin/main == 3e1049e`**); branch `f101b-narrator` + worktree RETIRED post-merge; DONE sentinel `.superpowers/handoffs/f101b-narrator-DONE.md`. A tool-less Opus narrator writes the day's GPU-market story as a structured artifact (`store/<cat>/story/YYYY-MM-DD.json`); the story page renders from it and falls back to the Phase A assembler when no valid artifact exists. Built subagent-driven (8 TDD tasks, fresh implementer + per-task spec+quality review + per-fix re-review each, whole-branch Opus review + a formal gate/linter-parity certification). **Shipped:** new `gpu_agent/narrator/` package (schema/store/inputs/prompt/gate/pin), a `narrator` CLI verb (emit/accept/fallback), artifact-first `build_story_model`, a DEDICATED narrator prompt pin (`fixtures/narrator/prompt-pin.json` — F6 baseline byte-untouched), run-cycle step 3(e3) + F83 lockstep re-record. **The gate is the SOLE quality mechanism (no scored eval bar by design).** Whole-branch Opus review found + CLOSED a live-page crash vector: the gate scanned fewer rendered fields than the build-time linter (`lint_story_copy`), and `build_site` RAISES (no fallback) on any lint hit — so a plausible gate-passing artifact (banned word in a chart label / related-doc title-outlet-date, the twice-rendered headline, or a single `index` while the gap-chart axis label spends the page's one-index budget) would crash the site build PERSISTENTLY. Closed across 3 fix passes (`ada46fb`/`3226a7b`/`7451620`) + Opus certification: **no schema-valid narrator answer passes the gate yet crashes the build.** Merged-suite gate **1884 passed / 6 skipped**; F6 pin + scoring v1 replay pin + F83 conformance + narrator pin all GREEN; `fixtures/evals`/`gpu_agent/evals`/`registry` byte-untouched across the whole branch. **KNOWN LIMITATIONS (surface):** `relatedDocs.title` is brain-authored + NOT structurally validated (the gather blob has no title field — banned words ARE caught, url ∈ pool + outlet == source ARE checked; a fabricated headline on a real link is not); `fixtures/narrator/hash-input.json` uses illustrative non-real series ids (harmless, would re-mint the pin to fix); minors (substring forward-marker check, triplicated hash recipe, prompt.py→story_model import smell) in the DONE sentinel. **NEXT (user-directed sequencing): DEPLOY — rebuild `site/` + push (F100 precedent) so the live page becomes the narrator story page; the live page still runs the OLD F100 dashboard until then.** First live narrated cycle (`fellBack:false`) is the POST-MERGE criterion (spec §8) — the next scheduled cycle proves it. After deploy: F96 (finding-id collision, promoted 3rd sighting) + F102 (price-sync `_yymmdd_date` crash on month-grain `--as-of`) fix lanes, then F101 Phase C (Explore layer; plan `85b0002`, build gated on B's merge — now unblocked). Standing prior state: the daily narrator was DESIGNED + PLANNED, docs-only, spec `1e83201` + plan `11c30b8`. Spec `1e83201` + plan-time amendment in `11c30b8` (`docs/superpowers/specs/2026-07-23-f101b-daily-narrator-design.md`), 8-task plan `11c30b8` (`docs/superpowers/plans/2026-07-23-f101b-daily-narrator.md`). **User decisions (interactive, zero AFK):** narrator SEES recent entries (yesterday's artifact + last 7 headlines); gate-failure ×2 → FALL BACK to the Phase A assembler (logged `narrator: fellBack`); **gates + pin only, NO scored eval bar** — realized at plan time as a DEDICATED narrator prompt pin (`fixtures/narrator/prompt-pin.json` + tripwire test; the F6 eval baseline stays byte-untouched, so this lane cannot redden F6 — spec §7 amended, flagged to the user). New `gpu_agent/narrator/` package (schema/inputs/prompt/gate/store/pin), CLI verb mirroring implication's emit/accept, artifact-first `build_story_model`, run-cycle sub-step 3(e3) + F83 lockstep re-record (its own task, F98 precedent). GATED-LANE RULES: no other prompt-affecting lane while open; MUST-NOT-TOUCH scoring.py/report.py/existing brains' prompts/gpu_agent/evals/fixtures/evals/registry-indicators; live narrated-cycle verification is a POST-MERGE criterion, not forced in-lane. Execution: subagent-driven in `.worktrees/f101b-narrator`, STOP before merge → `.superpowers/handoffs/f101b-narrator-DONE.md`; only the user merges. **USER-DIRECTED SEQUENCING: deploy (site rebuild + push, F100 precedent) happens AFTER Phase B lands — the live page still runs the OLD F100 dashboard until then.** F96 (promoted, third sighting) + F102 (price-sync crash; now front-page-relevant — the KPI band reads store/series) filed `75c775e`, queued for after B. Standing prior state: **F101 PHASE A — BUILT, REVIEWED + MERGED `d01bb11` (pushed 2026-07-23; merged-main suite 1810 passed / 6 skipped, both pins green; DONE sentinel in the retained `.worktrees/f101a-story-page`; NOT yet deployed to the live site by user direction).** Two daily cycles landed concurrently: v14 `e9cd797` and v15 `a96a599` (v15: Strong/improving, DMI 2.933 / SMI -1.080; suite 1751/6 at its commit; TWO defects logged → F96 promotion + F102). Housekeeping open: retire the merged `f101a-story-page` worktree+branch (copy its DONE sentinel to root handoffs first). Older standing state (2026-07-22, pre-build wording): the narrative-first category page ("Is supply catching up to demand?") is DESIGNED + PLANNED, docs-only. Spec `b8ecfd0` (`docs/superpowers/specs/2026-07-22-f101-narrative-page-design.md`), 9-task TDD plan `26d6696` (`docs/superpowers/plans/2026-07-22-f101a-narrative-page-renderer.md`), backlog **F101**. Full interactive visual-companion brainstorm — ALL decisions user picks, ZERO AFK (provenance table in the spec §12). Phase A = renderer-only skeleton on existing store data: new `gpu_agent/dashboard/{gap_chart,story_model,story_render}.py`, index.html swaps from the F97 brief to the story page; **frozen core / F6 / F83 / registry UNTOUCHED by design — this lane cannot trip the pins.** Execution mode user-selected: **subagent-driven-development in `.worktrees/f101a-story-page`** (fresh implementer per task + per-task spec+quality review + whole-branch Opus review; question-stop rule verbatim in every brief; STOP before merge — only the user merges). Phases B (daily narrator brain — F6+F83 gated) and C (Explore sub-pages + story archive) are SEPARATE future lanes, spec §10. **⚠ A scheduled daily cycle was LIVE mid-run when this handoff was written (2026-07-22 21:32, `work/daily-2026-07-22/` at extract stage, uncommitted `store/seen_docs.jsonl` +10 lines left untouched)** — it may park on commit because docs commits moved HEAD (v10/v13 precedent: benign, finalize per those entries); do NOT sweep its `store/` delta into any commit. Standing prior state: **F100 MERCHANT-GPU DASHBOARD REVAMP — BUILT, MERGED + PUSHED (LIVE)** 2026-07-21 — the merchant-GPU category page deep-dive dashboard revamp is BUILT and merged to main. Merge `2e1effa` (`--no-ff`, pushed; **`main == origin/main`**); 13 build commits `93e7e8f`→`aa2deac`, branch `f100-dashboard` + worktree RETIRED post-merge. Built subagent-driven (12 TDD tasks, fresh implementer + per-task spec+quality review each, whole-branch Opus review) per the plan. **Shipped:** a scannable dashboard — 2-sentence brief, 5 dynamic KPI cards (DMI/SMI excluded, they live in the chart), demand-vs-supply dual-line SVG chart, clickable six-dimensions list, and a slide-in "why" panel (ONE self-contained inline `<script>` — user-approved scoped relaxation of the F95 no-scripting convention, category page only) opening rationale/evidence/trend/confidence, with TSMC implications + standing calls FOLDED into the panels and a per-topic "full page →" link to the appendix `#dim-` anchor. New modules `gpu_agent/dashboard/deepdive_model.py` + `deepdive_render.py`; modified `brief_model.py`/`brief_render.py`/`site_build.py` + tests/dashboard. **Renderer/copy-layer only:** frozen core (`scoring.py`/`report.py`/brains/eval fixtures/`registry/indicators.json`) UNTOUCHED, no run-cycle step (no F83 change), no brain prompts — F6 pin + scoring v1 replay pin GREEN & UNMOVED throughout. Whole-branch Opus review: READY TO MERGE WITH FIXES — 1 Important (panel evidence-link XSS: a store URL placed into an `href` via a client escaper that misses double-quotes) FIXED with `encodeURI` + regression test, re-reviewed clean; deferred Minors logged in the DONE sentinel. Records: `.superpowers/handoffs/f100-dashboard-DONE.md`, spec `1c93f68`/plan `7aab4d6`, backlog F100. **✓ v13 FINALIZED + F100 DEPLOYED LIVE (this session, on the user's request):** the concurrent v13 cycle (finished ~2026-07-20 21:54, parked uncommitted, run by ANOTHER instance) was finalized — `2026-07-v13.json` (Strong, DMI 2.153 / SMI -0.380, 6 dims / 126 findings) registered in the scoring v1 replay pin (W_CURRENT, post-F60 per the v7..v12 precedent, verified replays EXACTLY dmi 2.153333 / smi -0.380000) and its `store/` output (scorecard + 8 findings + 4 wiki entity pages + dedup/seen/wiki-log appends) committed (`560b1d5`); no cycle-log entry existed from the original run. Then the committed `site/` was rebuilt with the F100 renderer against v13 and pushed (`675a6ea`), so the merchant-GPU page is LIVE as the new deep-dive dashboard at **revision 13** (category page + stylesheet only; Cloudflare auto-redeploys from main). Full suite **1749 passed / 6 skipped / 0 failed**; F6 pin + scoring v1 replay pin GREEN. `main == origin/main == 675a6ea`. (The v1-replay tripwire that briefly reddened the F100 merge is now cleared — it was the unregistered v13, never an F100 issue.) Records: `docs/superpowers/specs/2026-07-20-merchant-gpu-deep-dive-dashboard-design.md`, `docs/superpowers/plans/2026-07-20-f100-merchant-gpu-deep-dive-dashboard.md`, `.superpowers/handoffs/f100-dashboard-DONE.md`, backlog F100. Standing prior state: **F98 PART B MERGED** 2026-07-20 (merge `6b0bf37` `--no-ff`, pushed) — S4 `upstreamLeadTimes` adopted as a SCORING supply indicator (weight 0.12, dimension bottleneck / side supply / readsLevelOrSlope slope, `cadenceHorizon` weekly-leading) — built subagent-driven on branch `f98b-s4-leadtimes` (worktree `.worktrees/f98b-s4-leadtimes`), commits `bd3b7b8`→`a636ca4`. THE GATED LANE cleared by the book: ONE `registry/indicators.json` key (prompt-affecting, seam-scoped to EXTRACT only — verified char-level: one +87-char indicator line; judge/thesis/implication byte-identical) → F6 pin red BY DESIGN → eval-driver gate: eval PASS (r1 clean 7.00 vs bar 6.285; r1+r2 mean 6.688), 3-run governance rebaseline (`4d48a27`, NO `--force`, dispersion 0.875<1.0 → extract seamMean 6.500 eps 0.901 bar 5.599); scoring v1 replay pin + F6 pin GREEN throughout; manifest source `upstream-component-leadtimes` + binding-constraint slot added. Whole-branch Opus review: **READY TO MERGE** (0 Critical/Important). Full suite **1724 passed / 7 skipped**. **User decisions (interactive, NOT AFK):** (1) cadenceHorizon = weekly/leading — the spec's "one new key" MISSED that a scoring indicator MUST carry a cadenceHorizon tag (horizon.py::validate_coverage + tests enforce it); horizon=leading spec-dictated, weekly = user pick (sibling `leadTimes`); (2) a disclosed BORDERLINE pass (clean PASS rode the r1 draw; r2+r3 alone would marginal-fail the old bar); (3) **F79 canary PARKED** — the rebaseline widened the extract noise band (bar 6.285→5.599) below the F79 seeded-regression canary's 6.25 damaged score, so that "gate has teeth" test failed; re-run replicates (Option 1) proven futile and re-capture-harder (Option A) blocked BY DESIGN (the safety guard blocks running the eval on an edited prompt + blocks agent self-granting), so the user chose **Option B**: ship S4, skip the canary test (documented), file re-capture as **F99**. **⚠ INTERIM (until F99):** the extract SCORING bar is looser and the canary meta-proof is OFF; the F6 hash pin still catches ANY prompt change. **Live-extraction of an `upstreamLeadTimes` finding is verified on the NEXT scheduled cycle (spec criterion 6), NOT forced in-lane.** MUST-NOT-TOUCH (scoring.py/report.py/brains/eval-harness/other registry entries) honored. Records: `.superpowers/handoffs/f98b-s4-leadtimes-DONE.md` + `docs/superpowers/eval-notes/2026-07-18-f98b-upstreamLeadTimes-regate-note.md` + `docs/fix-backlog.md` F98(Part B)/F99. **MERGED `6b0bf37` (`--no-ff`) + pushed 2026-07-20 after a green merged-suite gate (**1726 passed / 6 skipped**; F6 pin + scoring v1 replay pin green). `main == origin/main`. Housekeeping now open: retire the `f98b-s4-leadtimes` branch + worktree.** Standing prior state: **F98 PART B DESIGNED** 2026-07-17 — spec `a40f77f` + plan `09c943e` (5 tasks) shipped; then BUILT 2026-07-18 (above) per the F98b coordination entry below. Standing prior state: **F98 PART A MERGED** 2026-07-17 — agenda-band data-readiness merged `--no-ff` to main (**merge `7e2f657`**, + this close-out docs commit; pushed 2026-07-17) after a green merged-suite gate. Built subagent-driven (9 TDD tasks, fresh implementer + per-task spec+quality review each, opus whole-branch review) on branch `f98-agenda-data`. **Shipped:** a `price-sync` CLI verb + run-cycle step turning the local gitignored `gpu_agent/data/gpu_leasing_data/` folder into `store/series/*.jsonl` — `gpuSpotPrice` (hardware street price, curated per-GPU, latest generation) + `gpuRentalOnDemand`/`gpuRental1yr` (USD/hr); **DISPLAY-ONLY, never scoring** (verified: price ids absent from `registry/series-indicators.json`); curated `registry/price-benchmarks.json` trust boundary (only named rows enter; adding a Rubin `rank:4` block later is a pure data edit); slot-family fixes (`registry/agenda-slots.json` — S9→customer-mix; price/apiArr/releaseCadence slotted); unit hygiene on the brief agenda band (`$500B`, `loosening`, plain labels, `[DSPX]\d` tile-label lint, money-unit change-line); manifest sources for apiArr/releaseCadence. **Full suite 1724/6, F6 pin + F83 conformance green, frozen core untouched.** **User decisions (interactive, NOT AFK):** price tiles insist on the newest chip (dim when its price is stale — no roll-down added); apiArr/releaseCadence manifest priority = optional. **Known non-blockers:** empty spot series (source `aws_spot_price.csv` has no GPU rows); dormant rental graceful-degradation roll-down (B200 rentable now, so no visible effect). Records: `.superpowers/handoffs/f98-agenda-data-DONE.md` (in the retained worktree) + `docs/fix-backlog.md` F98 + the F98 coordination entry below (now MERGED). **Housekeeping now open:** retire the merged `f98-agenda-data` branch + worktree (holds the gitignored DONE sentinel + SDD ledger) — plus the still-open `f97-exec-brief` retirement. **Part B (S4 upstream lead-time, F6-gated) stays a SEPARATE lane.** `main == origin/main` at the F98 merge `7e2f657` + this close-out docs commit (both pushed 2026-07-17). Standing prior state: **F97 EXECUTIVE-BRIEF MERGED + DEPLOYED (LIVE); 2026-07-17 v10 daily cycle FINALIZED** — the exec brief (blocks A–H) is the category page, built subagent-driven and merged `--no-ff` (`d2523a2`); the committed `site/` was then rebuilt with the F97 renderer against the **new v10 scorecard** and pushed (`1eaf365`), so the revamped page is LIVE (Cloudflare auto-redeploys from main; index = revision 10, all 12 evidence anchors resolve). Renderer/copy layer + one user-approved fix so all three latest-scorecard selectors (`scorecards.py`, `build.py`, `site_model.py`) prefer the monthly deep-read; `report.py` frozen-core untouched. **No open blockers.** The 2026-07-17 daily cycle (**Strong/improving, DMI 2.020 · SMI 0.007**) was run by a concurrent headless instance that completed its full pipeline but PARKED before committing — most likely because the F97 merge moved main's HEAD under it; this session verified its cycle-log (gates clean; standard AFK-defaults incl. the known F96 price-collision handled by logged exclusion), registered v10 in the scoring replay pin (W_CURRENT, v7/v8/v9 precedent, replays exactly), and committed+pushed the cycle (`a71343c`) plus the site rebuild (`1eaf365`). Full suite **1705 passed / 5 skipped**; F6 pin + F83 conformance green. Records: the F97 coordination entry below + `docs/fix-backlog.md` F97 + `.superpowers/handoffs/f97-exec-brief-DONE.md` (in the retained `f97-exec-brief` worktree). **Housekeeping now open:** retire the merged `f97-exec-brief` branch + worktree (holds the gitignored build ledger + DONE sentinel). `main` reached `1eaf365` (2026-07-17: F97 merge `d2523a2` + v10 cycle `a71343c` + site rebuild `1eaf365`, all pushed; f107748 pushed 2026-07-16: 4 F97 docs commits `2d00554…df3936f`, the parked v9 cycle `bb594a5` per its own "push when ready" note, the v9 replay-pin registration, and this handoff), suite **green post v9-pin registration** (2026-07-16 full run: 1664/1F/5 → the 1F was the v1-replay tripwire demanding deliberate registration of the new `2026-07-v9.json`; registered per the v7/v8 precedent, pin file 28/28 incl. v9 exact-replay, F6 pin green; final full-suite count in the session bullet below). Standing prior state: **F79 SCORING v2.0 SHADOW-MERGED** 2026-07-15 — `b6db80a` (frozen-core migration, SHADOW-ONLY: v2 computes but NOTHING user-facing renders it; v1 remains the headline; repo MIGRATED 2026-07-15 to the private **ai-market-digital-twin** repo; **F95 market-site MERGED** `2725578`), F6 pin green (4 seams). All four F79 gates cleared: G1 backfill SIGNED, G2 backtest PASSED (user-signed Option-B event amendment; detection-only; original FAIL reproducible), G3 eval re-gate PASSED (seam-scoped — only extract bound; honest baseline; canary captured), final review READY (replay fidelity + shadow isolation airtight). **REMAINING for F79: shadow soak ≥5 live cycles (manual `v2-shadow` invocation per cycle; auto-hook deferred) → G4 CUTOVER (user-signed; flips v1→v2 rendering) — the last gate.** Two findings logged this session: **F96** (monthly-grain write-back collision — the live v8 cycle's corpus write-back rolled back on a same-month price re-gather id collision, F52-class residual) and the F83 journal-conformance test was RELAXED to the real contract (`gates` optional; `stageStatuses` accepted — it had silently reddened main since v8; user-approved). Concurrent: the F88 session is active (unattended-orchestrator hardening; it ran the live v8 cycle; its uncommitted `.gitignore` line was preserved as `cf79758`). Next after F79's G4: F66, the F81–F86/F88–F95 waves. Housekeeping open: retire ~13 merged worktrees/branches (incl. f95-market-site), skipped-days decision, F23-A4 label, F73 survivorship-bias residual, OLD public random_for_fun repo disposition (make private/delete). Repo-rename/exposure gate RESOLVED 2026-07-15 — migrated to the private ai-market-digital-twin repo.)
...
- **F115 lane CLAIMED-FOR-DISPATCH (2026-08-10):** the next instance builds F115 in
  `.worktrees/f115-issue-tracker`, branch `f115-issue-tracker`, per plan `94a5885`.
  **EXCLUSIVE NARRATOR-PROMPT LANE** — while it is open, NO other lane may touch
  `gpu_agent/narrator/` or `fixtures/narrator/` (the pin moves once, in its Task 4).
  It also re-records F83 (Task 9) — no other run-cycle-step lane concurrently.
  STOP before merge → `.superpowers/handoffs/f115-issue-tracker-DONE.md`; only the user merges.
