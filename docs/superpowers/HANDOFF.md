...
- **F98b lane READY TO DISPATCH (2026-07-17, design session; execution assigned to a NEW instance by the user). THE GATED LANE — while it runs, NO other prompt-affecting lane (registry/prompt/eval changes) may be active.**
  CLAIM: branch `f98b-s4-leadtimes`, worktree `.worktrees/f98b-s4-leadtimes` — create on claim (does not exist yet).
