...
- **⚠ ROOT CHECKOUT CLAIMED: v8-cycle-resume lane (2026-08-20, unstarted).** The root working
  tree holds the interrupted 2026-08-19 run's UNCOMMITTED output (v8 scorecard, findings, wiki,
  coverage, first F115 issue register) plus its `work/daily-2026-08-19/` artifacts. The next
  instance resumes the cycle at the thesis seam ON THE ROOT CHECKOUT (a daily cycle runs there by
  design, not in a worktree). Until that cycle is committed + pushed: no other instance touches
  `store/`, `site/`, `tests/test_scoring_v1_replay_pin.py`, or runs a gather. `main == origin/main`
  apart from this HANDOFF commit; expect the scoring-replay tripwire red until v8 is registered.
