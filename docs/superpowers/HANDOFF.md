...
- **Date: 2026-07-16/17 — F97 EXECUTIVE-BRIEF RENDERER BUILT (subagent-driven), pushed on branch, NOT merged.**
  ...
  AFK-defaults.** **⚠ ONE OPEN PRE-MERGE DECISION (whole-branch review Important #1):** evidence links
  dead-end on the real store (brief=monthly vs appendix=daily via `load_scorecards` mixed-grain sort; fails
  criterion 6) — recommended fix is `scorecards.py`/F95 scope, surfaced to the user, NOT fixed AFK. Records:
  `docs/fix-backlog.md` F97 + `.superpowers/handoffs/f97-exec-brief-DONE.md`. **Branch pushed; only the user
  merges.**
...
- **F97 lane DONE ON BRANCH — PUSHED, AWAITING USER MERGE (built 2026-07-16/17, subagent-driven; only the user merges).**
  ...
  **⚠ ONE OPEN PRE-MERGE DECISION (whole-branch review Important #1):** the brief's evidence links dead-end on the
  real store — it anchors on the monthly deep-read while the F95 appendix builds from a stale daily
  (`load_scorecards` string-sorts `2026-07-06` above `2026-07`), so 5 `#f-` + 2 of 6 `#dim-` links point at missing
  anchors (fails spec acceptance criterion 6). Recommended fix (prefer the monthly read as "latest") touches
  `scorecards.py`/F95 scope, OUTSIDE this lane's ownership — surfaced to the user, NOT fixed AFK. Records:
  `.superpowers/handoffs/f97-exec-brief-DONE.md` + `docs/fix-backlog.md` F97. Original claim/ownership below (now built).
