# HANDOFF — GPU Category Agent (resume point: **2026-08-25 (late session) — F127 BUILT, PARKED UNMERGED on branch `f127-excerpt-length` (worktree `.worktrees/f127-excerpt-length`, tip `bd49e0b`, 10 commits on `e6b9de3`). `main == origin/main == e6b9de3` plus this handoff commit. Lane suite 2765 passed / 6 skipped (+45 tests); F6 pin BYTE-UNTOUCHED (only product file changed: `gpu_agent/gate.py`). NOT merged because the lane rests on an AFK-default that changes the spec's letter: the backlog said a hard 50-word cap, the lane built 'reject only if BOTH >50 words AND >2 sentences, or >100 words absolute' after re-measuring the store (644 excerpts now, one 70-word single-sentence AMD 10-Q quote would fail a hard cap). USER MUST RULE: (a) accept the OR reading and merge, (b) demand the hard 50-word cap and re-work, or (c) something else. Six AFK-defaults listed in `.worktrees/f127-excerpt-length/.superpowers/handoffs/f127-excerpt-length-QUESTIONS.md`. New backlog candidate surfaced: `wiki/ingest.py::route_findings` writes findings to the store without running the gate at all (pre-existing, affects every gate rule). Still parked for interactive brainstorm: F125 honest-removal, F92 reference scorecards. NVIDIA earnings 2026-08-26 — next real cycle leads with it. Do not delete `work/eval-2026-08-24/`.**
