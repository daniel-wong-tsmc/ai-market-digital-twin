...
- **F115 lane BUILT + REVIEWED — AWAITING USER MERGE (2026-08-14).** Branch `f115-issue-tracker`
  (13 commits `9fd9807`→`acf5e30`, off merge-base `a197f93`), pushed to origin; worktree
  `.worktrees/f115-issue-tracker` retained. Sentinel written:
  `.superpowers/handoffs/f115-issue-tracker-DONE.md`. **Only the user merges — do NOT merge this.**
  **The exclusive narrator-prompt lock is RELEASED**: both permitted pins have already moved, each
  exactly once and deliberately (narrator prompt pin `cf304de`; F83 run-cycle fingerprint `337f82e`),
  and both were independently recomputed by reviewers. A future narrator- or run-cycle-touching lane
  may now proceed — but note it will REBASE ONTO THESE MOVED PINS, not onto main's older values.
  Branch state: pytest 2541 passed / 6 skipped, npm 164, build OK, forbidden diff vs `main` EMPTY.
  ⚠ Checks were taken against merge-base `a197f93`, and `main` HAS SINCE ADVANCED FOUR COMMITS (v6
  `80e54f0`/`a8f02d7`, v7 `7859898`/`5f2750d`) — re-verify after merge, and rebuild `dashboard.json`
  + `site/` in the same session (the app is strict at schema 1.2 and will refuse main's live 1.1 data).
