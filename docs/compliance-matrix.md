...
> repo-root-relative and must exist; a `path::name` reference must name a real function — prefer
> `file::test_name` pins so a renamed or deleted test fails the lint. An `ENFORCED` row must name at
> least one `tests/` path. **No line numbers** anywhere — reference a function or rule by name in
> parentheses (line numbers rot on every edit). `Status` is exactly one of:
...
productionization and Layer / Main tiers the charter itself marks "Not yet." The four `NOT-ENFORCED`
...
| P1.3 | 1 | State the impact (targets, direction, mechanism) | ENFORCED | gpu_agent/gate.py (F21 targets and mechanism non-empty) | tests/test_gate_v12.py::test_impact_targets_empty |
...
| P1.5 | 1 | Always state the source (name, url, date); primary over secondary | ENFORCED | gpu_agent/gate.py (F2a evidence required, ISO date) plus gpu_agent/gathering/ingest.py (tier stamped) | tests/test_gate_v12.py::test_observed_finding_missing_evidence |
| P1.6 | 1 | Label hypotheses; cap confidence; show reasoning | ENFORCED | gpu_agent/gate.py (hypothesis reasoning plus confidence cap) | tests/test_gate_finding.py::test_hypothesis_requires_reasoning_and_capped_confidence |
...
| P1.q1 | 1 | Confidence always present with a stated basis | ENFORCED | gpu_agent/schema/finding.py (confidence required) | tests/test_finding_schema.py::test_measured_finding_roundtrips |
| P1.q2 | 1 | Dispersion when sources disagree; never silently pick one | ENFORCED | gpu_agent/gathering/dedup.py (conflicting keys set dispersion) | tests/test_dedup_classify.py::test_classify_conflicting_batch_mates_set_dispersion |
...
| P2.observed | 2 | observed: value must be null, evidence required | ENFORCED | gpu_agent/gate.py (check_finding) | tests/test_gate_v12.py::test_observed_finding_missing_evidence |
| P2.hypothesis | 2 | hypothesis: value null, reasoning required, confidence at most medium | ENFORCED | gpu_agent/gate.py (check_finding) | tests/test_gate_finding.py::test_hypothesis_requires_reasoning_and_capped_confidence |
| P2.polarity | 2 | v1.1 demand/supply read: side set plus at least one non-zero polarity | ENFORCED | gpu_agent/gate.py (affects-neither-track check) | tests/test_gate_finding.py::test_finding_affecting_neither_track_fails |
| P2.schema | 2 | The Finding is the frozen atomic unit, schema-validated | ENFORCED | gpu_agent/schema/finding.py | tests/test_finding_schema.py::test_measured_finding_roundtrips |
| P3.scorecard | 3 | Scorecard = findings plus six dimension ratings that name their finding IDs | ENFORCED | gpu_agent/schema/scorecard.py plus gpu_agent/gate.py (ratings cite finding IDs) | tests/test_scorecard_schema.py::test_dimensions_are_the_six_fixed_ones plus tests/test_gate_scorecard.py::test_rating_with_no_citations_fails |
...
| P4.memory | 4 | Each tier reads prior state plus notebook and interrogates the change | ENFORCED | gpu_agent/memory.py (prior-state bundle into judge and thesis prompt) | tests/test_memory_bundle.py::test_build_memory_bundle_prior_category_status_and_indices |
| P4.series | 4 | State time-series persisted so compare-to-before is a query | ENFORCED | gpu_agent/store.py plus gpu_agent/wiki/store.py | tests/test_store.py::test_append_is_versioned_and_non_destructive |
| P5.chassis | 5 | Structured outputs plus pre-commit schema validation; read-prior write-snapshot | ENFORCED | gpu_agent/schema/finding.py plus gpu_agent/gate.py plus gpu_agent/store.py | tests/test_finding_schema.py::test_measured_finding_roundtrips plus tests/test_store.py::test_append_is_versioned_and_non_destructive |
...
| P6.frozen | 6 | Output schema frozen at each boundary; the next tier's only input | ENFORCED | gpu_agent/schema/finding.py plus gpu_agent/schema/scorecard.py | tests/test_scorecard_schema.py::test_pre_b_scorecard_still_validates_without_new_fields |
| P6.rollup | 6 | Code-computed DMI, SMI, SDGI travel up the contract | ENFORCED | gpu_agent/scoring.py | tests/test_scoring.py::test_dmi_smi_contribution |
| P7.a | 7 | Every measured Finding has a value and at least one dated source | ENFORCED | gpu_agent/gate.py (check_finding) | tests/test_gate_finding.py::test_measured_without_value_fails |
| P7.b | 7 | No observed or hypothesis Finding carries an invented value | ENFORCED | gpu_agent/gate.py (check_finding) | tests/test_gate_finding.py::test_observed_with_value_is_invented_number |
...
| P7.d | 7 | Every Finding has an impact (targets, direction, mechanism) | ENFORCED | gpu_agent/gate.py (F21) | tests/test_gate_v12.py::test_impact_targets_empty plus tests/test_gate_v12.py::test_impact_mechanism_empty |
| P7.e | 7 | Provenance (evidence) or, for a hypothesis, reasoning | ENFORCED | gpu_agent/gate.py (check_finding) | tests/test_gate_v12.py::test_observed_finding_missing_evidence plus tests/test_gate_finding.py::test_hypothesis_requires_reasoning_and_capped_confidence |
| P7.f | 7 | Every hypothesis labeled and capped at confidence medium | ENFORCED | gpu_agent/gate.py (check_finding) | tests/test_gate_finding.py::test_hypothesis_requires_reasoning_and_capped_confidence |
| P7.g | 7 | Conflicting sources surfaced as dispersion, not silently resolved | ENFORCED | gpu_agent/gathering/dedup.py | tests/test_dedup_classify.py::test_classify_conflicting_batch_mates_set_dispersion |
| P7.h | 7 | Every rating, stance, or status names the Finding IDs that justify it | ENFORCED | gpu_agent/gate.py (check_scorecard citations) | tests/test_gate_scorecard.py::test_rating_with_no_citations_fails |
| P7.i | 7 | No Finding cites the dashboard's own prior output (no self-reference) | ENFORCED | gpu_agent/gate.py (check_scorecard self-reference) | tests/test_gate_scorecard.py::test_self_reference_fails |
...
| P8.injection | 8 | Fetched content is data, not instructions | PARTIAL | gpu_agent/extraction/prompt.py (F16 delimiting) plus gpu_agent/gathering/webreach.py (validate_request refuses non-http(s) scheme, unknown tool, unknown verb; run_requests execs shell=False argv only, no page-controlled syntax) plus .claude/agents/web-gatherer.md (restricted gatherer agent type, tools Read/Write/WebSearch/WebFetch) plus SESSION-PROSE (receipts-not-content hand-off, .claude/skills/gather-category/SKILL.md) | tests/test_extraction_prompt.py plus tests/test_webreach_runner.py |
...
| P16.declarative | 16 | A new category is a taxonomy entry plus archetype selection, no code fork; the human-approval half is P16.humangate | ENFORCED | docs/taxonomy.json plus gpu_agent/registry/structure.py plus gpu_agent/assignment.py | tests/test_taxonomy_scope.py::test_all_categories_spans_layers_and_is_unique |
...
| P16.version | 16 | Taxonomy version bumped so every prior snapshot stays reproducible | NOT-ENFORCED | NOT ENFORCED - aspiration (docs/taxonomy.json has no live version field - the only version string describes assignment records - and taxonomyVersion appears nowhere in code) | — |
| P17.split | 17 | Measured facts and judgments kept apart; never invent a score | ENFORCED | gpu_agent/gate.py plus gpu_agent/scoring.py (no synthetic 0-100) | tests/test_gate_finding.py::test_observed_with_value_is_invented_number plus tests/test_scoring.py::test_dmi_smi_contribution |
| P17.scale | 17 | Five-word rating scale with written, repeatable definitions | ENFORCED | gpu_agent/bands.py plus docs/rating-anchors.md | tests/test_rating_anchors.py::test_each_dimension_section_has_all_five_ratings_exactly_once |
...
| P17.anchor | 17 | The measured anchor bounds the rating (bias guardrail), never sets it | ENFORCED | gpu_agent/gate.py (F36) | tests/test_gate_scorecard.py::test_rating_contradicting_anchor_fails |
| P17.cover | 17 | Alongside the status word always say the bottleneck, the direction, and the one-line reason | ENFORCED | gpu_agent/schema/scorecard.py (CategoryStatus requires rating, direction, bottleneck, reason) | tests/test_scorecard_schema.py::test_category_status_roundtrips |
...
| P18.contract | 18 | Small sacred fixed contract (6 dims, schema, scale, rollup); not overridable | ENFORCED | gpu_agent/schema/finding.py plus gpu_agent/schema/scorecard.py plus gpu_agent/scoring.py | tests/test_scorecard_schema.py::test_dimensions_are_the_six_fixed_ones |
| P18.defineonce | 18 | Define each thing once, refer by id (three registries) | ENFORCED | registry/indicators.json plus gpu_agent/registry/structure.py plus docs/taxonomy.json | tests/test_registry_structure.py::test_registry_validates_clean_against_taxonomy |
| P18.template | 18 | One template, many instances (assignment-driven scope) | ENFORCED | gpu_agent/assignment.py | tests/test_assignment_category.py::test_assignment_has_category |
...
| P18.modularity | 18 | Modularity binding: every component behind a named seam or registry | PARTIAL | Named seams are real and individually pinned (gpu_agent/llm/factory.py, gpu_agent/cycle.py, registry/web-reach-tools.json); the universal every-component property is architecture review, not code-enforced | tests/test_llm_backends.py |
...
| P19.nosilent | 19 | Never a silent partial; the run states what was run vs skipped | ENFORCED | gpu_agent/cycle.py (cycle log: run vs skipped and why) | tests/test_store_cycle_log_integrity.py::test_cycle_log_is_finalized_journal_not_plan_skeleton |
...
| P19.budget | 19 | Per-cycle budget ceiling plus load-shedding: a total cost ceiling per refresh, fast-movers first while slow-movers ride their TTL | DEFERRED | DEFERRED (no budget governor in v1; cost controls land with the Part 27 costed pilot - roadmap Phase 4) | — |
...
| P20.snapshot | 20 | Source snapshots captured plus content-hashed so citations survive link rot | ENFORCED | gpu_agent/schema/raw_document.py plus gpu_agent/gathering/ingest.py (content hash) | tests/test_raw_document.py::test_raw_document_roundtrips |
| P20.replay | 20 | Reproducible: re-running on the pinned inputs yields an equivalent result | ENFORCED | gpu_agent/llm/recorded.py | tests/test_replay_v12.py::test_v12_is_replay_of_v6 |
...
| P24.golden | 24 | A frozen human-curated golden set per archetype with a rubric | ENFORCED | gpu_agent/evals/cases.py plus gpu_agent/evals/harness.py | tests/test_evals_cases.py::test_case_parses_and_seam_input_typed |
| P24.gate | 24 | Prompt regression gate: no promptVersion reaches canonical below the incumbent | ENFORCED | gpu_agent/evals/prompt_hash.py plus gpu_agent/evals/harness.py | tests/test_evals_baseline_pin.py::test_prompt_hashes_match_baseline |
...
| P25.qualify | 25 | A new model or prompt clears the golden set before it may run canonical | ENFORCED | gpu_agent/evals/prompt_hash.py (hash pin blocks canonical) | tests/test_evals_baseline_pin.py::test_prompt_hashes_match_baseline |
...
| P26.privsep | 26 | Privilege separation at fetch; content can populate evidence, never emit tool calls | SESSION-PROSE | .claude/agents/web-gatherer.md (no-shell gatherer agent type) plus SESSION-PROSE (.claude/skills/gather-category/SKILL.md) | — |
...
| P33.frozen | 33 | The contract is frozen within a version; frozen-core changes ship as migrations | ENFORCED | gpu_agent/schema/finding.py (schemaVersion) plus the eval hash-pin | tests/test_evals_baseline_pin.py::test_prompt_hashes_match_baseline |
...
| P37.ingestseam | 37 | The ingest seam turns raw blobs into validated, de-duplicated, tier-stamped RawDocuments | ENFORCED | gpu_agent/gathering/ingest.py plus gpu_agent/schema/raw_document.py | tests/test_ingest.py::test_valid_blob_becomes_validated_rawdocument |
| P37.webreach | 37 | Web-reach tools are a data registry; their output is secondary; ensure-installed idempotently | ENFORCED | registry/web-reach-tools.json plus gpu_agent/web_reach_ensure.py | tests/test_web_reach_registry.py::test_registry_parses_and_has_version plus tests/test_web_reach_ensure.py |
...
| P37.trusttier | 37 | Trust tier stamped deterministically at ingest; a secondary-only finding is confidence-capped | ENFORCED | gpu_agent/gathering/ingest.py (tier stamp) plus gpu_agent/gate.py (F2e) plus gpu_agent/publisher.py | tests/test_ingest.py::test_open_web_is_secondary plus tests/test_gate_corroboration.py::test_two_distinct_publishers_still_rejected_with_count |
...
| P38.cyclelog | 38 | A cycle writes a replayable cycle log; a coverage gap is visible, never implied | ENFORCED | gpu_agent/cycle.py plus gpu_agent/store.py | tests/test_store_cycle_log_integrity.py::test_cycle_log_is_finalized_journal_not_plan_skeleton |
...
| P38.assignlog | 38 | A selected category with no assignment is logged as skipped, never silently dropped | ENFORCED | gpu_agent/assignment.py plus gpu_agent/cycle.py | tests/test_cycle_plan.py::test_build_cycle_plan_marks_ready_skipped_and_stages plus tests/test_cli_cycle_plan.py::test_cycle_plan_layer_surfaces_skipped_on_stderr |
| P39.wiki | 39 | LLM-wiki threads: code-owned header, brain-curated body, cited finding history | ENFORCED | gpu_agent/wiki/store.py plus gpu_agent/wiki/page.py plus gpu_agent/wiki/log.py | tests/test_wiki_store.py::test_update_header_disallowed_field_raises |
| P39.indices | 39 | Momentum (trailing) and Outlook (forward) indices computed in code, additive fields | ENFORCED | gpu_agent/registry/horizon.py plus gpu_agent/registry/tracks.py plus gpu_agent/scoring.py | tests/test_registry_horizon.py::test_validate_coverage_passes_for_real_registry |
...
| P39.momentum | 39 | Salience and materiality computed in code, never brain-invented; every drop logged | ENFORCED | gpu_agent/wiki/salience.py plus gpu_agent/wiki/lint.py | tests/test_wiki_lint_materiality.py::test_score_contradiction_highest |
...
1. **P21.resolve — entity alias canonicalization and per-category namespacing (Part 21, Part 18 registry).**
   ...
   Part 21 says a multi-category entity "must be counted once." Reality: `docs/taxonomy.json` embeds
   `seedEntities` carrying exactly those fields, but **no code consumes them** — there is no
   alias-resolution or reconciliation pass, so `NVDA` and `nvidia` are not reconciled and the entity id
   is global. Already tracked as **F24** (open, Should-have). This is the closest analogue to the
   F2/F3/F5 pattern: a Part-18 "fixed contract" element that is, today, aspiration with seed data.
2. **P16.version — taxonomy version bump (Part 16).** The charter says the taxonomy version is bumped so
   every prior snapshot stays reproducible. Reality: `docs/taxonomy.json` has **no live version field**
   (the only "version" string in the file is schema description for assignment records), and
   `taxonomyVersion` appears nowhere in code or tests. Nothing to bump, nothing that reads it.
3. **P23.access — role-based human access + data classification (Part 31/23).** No access control or
   classification exists in v1. Part 31 frames the store as confidential CI; the control is absent.
4. **P31.secrets — vault, encryption at rest, audit log (Part 31).** API keys live in environment
